/* Castle Duel v57. Decisions use visible balloons, own ammo and shot history only. */
var CastleBot57 = (() => {
  ...
  function create(side, level, rng = Math.random) {
    ...
    function next(obs) {
      ...
      for (const b of enemy) {
        ...
        if (!v) continue;
        if (rng() < p.defend) return noisy({ ...v, weapon: 'arrow', balloonId: b.id });
        break;
      }
  ...
  return { profiles, create, placements, placementFromShot, velocity };
})();
