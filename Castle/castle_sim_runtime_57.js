// Appended INSIDE the actual game's script by castle_simulator_57.mjs.
// Only transport, the clock, rendering and the automated human's input are substituted.
let simBotRows = [], simStats, simHuman, simHumanBomb = null;
...
function simObservation(side) {
  ...
  return { ammo: side === 1 ? ammo : botAmmo, ownHP: castleHP[side], enemyHP: castleHP[side === 1 ? 2 : 1],
    balloons: balloons.filter(b => b.alive).map(b => ({ id: b.id, owner: b.owner, ...balloonPos(b) })) };
}
async function simHumanShoot() {
  if (turn !== 0 || q.fly || gameOverWinner) return;
  let action = simHuman.next(simObservation(1));
  if (action.pass) action = { weapon: 'arrow', ...CastleBot57.velocity(1, { x: 800, y: 390 }, 'arrow') };
...
async function simRun(seed, level, options = {}) {
  env.reset(); Math.random = makeRng(seed);
  if (options.profileOverrides) for (const [key, value] of Object.entries(options.profileOverrides)) Object.assign(CastleBot57.profiles[key], value);
  simStats = { shots: { 1: { arrow: 0, cannonball: 0, bomb: 0, rocket: 0 }, 2: { arrow: 0, cannonball: 0, bomb: 0, rocket: 0 } } };
  ...
  simHuman = CastleBot57.create(1, reference, Math.random);
  ...
  const human = CastleBot57.placements(1, reference, sizes, Math.random);
...
async function simChecks() {
  ...
  env.reset(); Math.random = makeRng(481516); simStats = { shots: { 1: {}, 2: {} } }; soundOn = false;
  ...
  for (const side of [1, 2]) for (const level of ['easy', 'medium', 'hard', 'reference']) for (let trial = 0; trial < 100; trial++) {
    const objects = CastleBot57.placements(side, level, sizes, Math.random);
  ...
  for (const side of [1, 2]) for (const weapon of ['arrow', 'cannonball', 'bomb', 'rocket']) for (let y = 80; y <= 700; y += 60) for (let x = 60; x < 800; x += 60) {
    const target = { x: side === 2 ? x : VW - x, y }, shot = CastleBot57.velocity(side, target, weapon);
    if (!shot) continue;
  ...
  for (const level of ['easy', 'medium', 'hard']) {
    const brain = CastleBot57.create(2, level, makeRng(616));
  ...
  simHuman = CastleBot57.create(1, 'reference', makeRng(3));
...
return { run: simRun, checks: simChecks, inspect: () => ({ targetSizes: targetImgs.map((_, i) => placementSpriteSize(i)), physicsStep: PHYS_STEP, profiles: CastleBot57.profiles }) };
