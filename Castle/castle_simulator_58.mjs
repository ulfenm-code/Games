...
const sourcePath = path.resolve(ROOT, arg('--source', 'castle_index_58.html'));
const html = fs.readFileSync(sourcePath, 'utf8');
const source = [...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)].map(m => m[1]).filter(Boolean).join('\n');
const botFile = html.match(/<script src="(castle_bot_\d+\.js)"/)[1];
const botName = 'CastleBot' + botFile.match(/\d+/)[0];
const bot = fs.readFileSync(path.join(ROOT, botFile), 'utf8');
const runtime = fs.readFileSync(path.join(ROOT, 'castle_sim_runtime_58.js'), 'utf8');
const arrowChecks = fs.readFileSync(path.join(ROOT, 'castle_arrow_checks_58.js'), 'utf8');
...
const env = {
  ...
  clearTimeout: id => timers.delete(id), setInterval: fn => { if (!env.allowPoll) throw new Error('Network polling forbidden in simulation'); env.poll = fn; return 1; }, clearInterval: noop,
...
const factory = new Function('env', `const {${globals}}=env;\n${bot}\nconst CastleBotUnderTest=${botName};\n${source}\n${arrowChecks}\n${runtime}`);
const engine = factory(env);
if (args.includes('--arrow-checks')) { console.log(JSON.stringify(await engine.arrowChecks(Number(arg('--scenes', '500'))))); process.exit(0); }
...
for (const level of levels) {
  const totals = { level, matches: count, botWins: 0, humanWins: 0, timeouts: 0, botObjectWins: 0, botCastleWins: 0, humanObjectWins: 0, humanCastleWins: 0, meanSeconds: 0, meanTurns: 0, botArrows: 0, humanArrows: 0, botFriendlyBalloonHits: 0, botEnemyBalloonHits: 0, botGamesWithFriendlyHits: 0 };
  for (let i = 0; i < count; i++) {
    ...
    totals.botFriendlyBalloonHits += result.botFriendlyBalloonHits; totals.botEnemyBalloonHits += result.botEnemyBalloonHits; if (result.botFriendlyBalloonHits) totals.botGamesWithFriendlyHits++;
...
const out = path.resolve(ROOT, arg('--out', 'castle_simulation_58.json'));
