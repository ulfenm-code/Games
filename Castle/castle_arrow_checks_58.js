...
async function simArrowChecks(randomScenes) {
  ...
  const totals = { sourceVersion: guarded ? 58 : 57, scenes: 0, arrowsChecked: 0, ownHits: 0, enemyHits: 0, misses: 0, nonArrowChoices: 0, knownCases: [], timingCases: ['60 fps', '25 fps', '30 fps + 1 s launch delay'] };
  ...
  started = true; gamePhase = 'battle'; botMode = true; deviceRole = 1; turn = 1;
  ...
  simTransportOverride = path => {
    ...
    if (path.startsWith('shots?') && path.includes('turn_no=gt.')) return [{ turn_no: 10, player: 2, vx: -650, vy: -900, weapon: 'arrow' }];
    return [];
  };
  ...
  botMode = false; lastSeenTurnNo = 9;
  await env.poll();
