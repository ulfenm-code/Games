/* Castle Duel v58. Decisions use visible balloons, own ammo and shot history only. */
var CastleBot58 = (() => {
  ...
  const profiles = {
    ...
    medium: { defend: .75, error: .032, memory: .65, explore: .28, delay: 2.4, spread: .7, aerial: .45, bombSkill: .65, shelter: .07 },
    hard: { defend: 1, error: .006, memory: 1, explore: .05, delay: 1.3, spread: 1, aerial: .75, bombSkill: 1, shelter: .9 },
  ...
  // Same collision rectangle as arrowBalloonHit(), plus room for frame timing.
  ...
  function arrowPathStatus(side, shot, balloons) {
    ...
    for (let n = 1; n <= 841; n++) {
      ...
      // Reject ambiguous overlaps even if the enemy might be tested first by the engine.
      if (own.some(b => intersects(b, 18, 10))) return 'friendly';
      if (enemy.some(b => intersects(b, 0, 0))) reachesEnemy = true;
      // Check beyond the intended impact too, in case timing makes the arrow miss.
  ...
  function velocity(side, target, weapon, movingVx = 0) {
    ...
    for (let n = 42; n <= 300; n += 2) {
      ...
      // The game uses semi-implicit Euler: include its half-step gravity term.
  ...
  function create(side, level, rng = Math.random) {
    ...
    function next(obs) {
      ...
      for (const b of enemy) {
        ...
        // Freeze the benchmark opponent so it remains comparable with v57.
        if (level === 'reference') return first;
        const accept = shot => {
          // Keep arrows away from the edge where new friendly balloons enter.
        ...
        // Check AFTER all aim variation and speed clamping have been applied.
        ...
        // Try another enemy; otherwise use ground weapons or pass this turn.
      }
      ...
      for (const { cell } of ranked) {
        ...
        // Save straight-flying rockets for high or distant targets when possible.
    ...
    return { next, observe, hit(x, y) { hitPoint = { x, y }; }, delay() { return p.delay + rng() * 1.2; }, get actions() { return actionNo; } };
  }
