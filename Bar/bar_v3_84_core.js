...
const barTalkHeadState384={active:false,saved:'neutral',timer:null,current:null};
function barTalkDelay384(){return 350+Math.random()*300}
function barTalkNext384(current){
...
function barTalkTick384(){
  const s=barTalkHeadState384;if(!s.active)return;
  const api=window.__barSceneConfig384;
  if(!api?.setHeadVariant){
    s.timer=setTimeout(barTalkTick384,120);
    return
  }
  const current=api.currentHead?.()||s.current||s.saved||'neutral';
  const next=barTalkNext384(current);
  if(api.setHeadVariant(next)){
    s.current=next;
    s.timer=setTimeout(barTalkTick384,barTalkDelay384())
  }else{
    s.timer=setTimeout(barTalkTick384,120)
  }
}
function startTalkingHead384(){
  const s=barTalkHeadState384;if(s.active)return true;
  const api=window.__barSceneConfig384;
...
function stopTalkingHead384(){
  const s=barTalkHeadState384;
  if(s.timer){clearTimeout(s.timer);s.timer=null}
  if(!s.active)return;
  s.active=false;
  const restore=s.saved||'neutral';
  s.saved='neutral';s.current=null;
  window.__barSceneConfig384?.setHeadVariant?.(restore)
}
window.__barSpeechHead384={start:startTalkingHead384,stop:stopTalkingHead384,state:barTalkHeadState384};
...
function stopSpeech(options={}){
  ...
  if(!keepTalkingHead)stopTalkingHead384()
}
async function barPlayTtsV337(text,options={}){
  ...
  const notifyPlayback=()=>{
    ...
    if(manageHead){startTalkingHead384();headStarted=true}
  ...
  try{
    ...
    if(manageHead&&headStarted)stopTalkingHead384()
  }
}
...
(function(){
  ...
  function activateWhiteRumV347(){
    ...
    if(correctQuestion){
      /* v3.84: the correct White Rum selection is shown in-scene.
         Do not cover the bartender with the old full-screen reward clone. */
      if(view){view.overlay.remove();view.source.style.visibility=''}
      const animation=window.__barWhiteRum384?.playCorrect;
