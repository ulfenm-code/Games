'use strict';
(function(){
  const START_CONFIG_URL='./bartender_installningar_start6.json';
  ...
  function liquidHeight(){
    ...
    if(visual.rum)return 92;
    if(visual.ice)return 89;
    if(visual.littleSoda)return 70;
    if(visual.limeSugar)return 64;
    if(visual.mint)return 48;
    return 0;
  }
  ...
  function ensureIcePattern(){
    ...
    for(let i=0;i<28;i++){
      icePattern.push({
        ...
        h:4+Math.random()*5,
        r:-38+Math.random()*76,
        opacity:.42+Math.random()*.36,
        c1:2+Math.random()*9,
        c2:88+Math.random()*10,
        c3:88+Math.random()*10,
        c4:2+Math.random()*9
      });
    }
  }
  function renderIcePattern(layer){
    ...
    icePattern.forEach(b=>{
      ...
      cube.style.clipPath='polygon('+b.c1+'% 0,'+b.c2+'% 0,100% '+b.c3+'%, '+b.c4+'% 100%,0 18%)';
  ...
  const sceneConfig=window.__barSceneConfig376;
  ...
  async function showEmptyHighball(){
    ...
    clearGarnish();
    if(sceneConfig?.loadAndApply){
      await sceneConfig.loadAndApply(HIGHBALL_CONFIG_URL,{showGlass:true});
    }else{
  ...
  async function showMojitoGarnish(){
    ...
    if(sceneConfig?.loadAndApply){
      await sceneConfig.loadAndApply(GARNISH_CONFIG_URL,{showGlass:true});
    }else{
