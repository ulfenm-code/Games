...
const BAR_TTS_INSTRUCTION_V337='Tala naturlig svenska, varm manlig bartender-röst, avslappnad och vänlig.';
...
function createStreamingSpeechV119(){
  ...
  function queue(text){const s=String(text||'').trim();if(!s||cancelled)return;chain=chain.then(()=>cancelled?undefined:barPlayTtsV337(s)).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI streaming TTS',e)})}
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  ...
  try{
    ...
    const r=await fetch(BAR_AI_URL_V119,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({message,previous_response_id:state.aiPreviousResponseId,game:context,voice_instruction:BAR_TTS_INSTRUCTION_V337}),signal:controller.signal});
    if(!r.ok){
...
renderStep=function(newStep=false){
  ...
  speakRecipeIntentV337(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token})
};
...
answerKnownChoice=function(answer,btn){
  ...
  btn?.classList.add('good');completeRecipeStep(s,false)
};
...
completeRecipeStep=function(s,withPraise=false){
  ...
  if(withPraise){
    ...
    speakRecipeIntentV337(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token},()=>advanceStep(s));
    return
  }
  advanceStep(s)
};
...
advanceStep=function(s){
  ...
  if(s?.afterAi&&state.drink?.id==='mojito'){
    ...
    speakRecipeIntentV337(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase==='recipe'&&state.drink?.id==='mojito')renderStep(true)});
    return
  }
  renderStep(true)
};
...
orderDrink=function(d){
  ...
  speakRecipeIntentV337(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase!=='recipe'||state.drink!==d)return;if(state.difficulty==='hard')askServing();else renderStep(true)})
};
...
handleConversation=async function(raw){
  ...
  try{
    ...
    voice.finish();setAiBusyV119(false);streamDialogV119(turn.reply||'Jag hänger med.');await voice.done;
