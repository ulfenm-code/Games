...
function createStreamingSpeechV119(){
  ...
  stopSpeech();startTalkingHead380();
  function resolveVoice(){if(resolved)return;resolved=true;stopTalkingHead380();resolveDone()}
  function maybeDone(){if(!resolved&&streamDone){chain.finally(resolveVoice)}}
  function queue(text){const s=String(text||'').trim();if(!s||cancelled)return;chain=chain.then(()=>cancelled?undefined:barPlayTtsV337(s,{manageHead:false})).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI streaming TTS',e)})}
...
renderStep=function(newStep=false){
  ...
  speakRecipeIntentV337(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token},()=>{
    if(state.phase==='recipe'&&state.drink&&currentStepV119()===s&&!state.aiBusy)renderRecipeControlsV119()
  })
};
...
advanceStep=function(s){
  ...
  if(s?.afterAi&&state.drink?.id==='mojito'){
    ...
    speakRecipeIntentV337(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{
