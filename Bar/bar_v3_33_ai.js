...
const BAR_TTS_INSTRUCTION_V333='Tala naturlig svenska, varm manlig bartender-röst, avslappnad och vänlig.';
...
function createStreamingSpeechV119(){
  ...
  function queue(text){const s=String(text||'').trim();if(!s||cancelled)return;chain=chain.then(()=>cancelled?undefined:barPlayTtsV333(s)).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI streaming TTS',e)})}
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  ...
  try{
    ...
    const r=await fetch(BAR_AI_URL_V119,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({message,previous_response_id:state.aiPreviousResponseId,game:context,voice_instruction:BAR_TTS_INSTRUCTION_V333}),signal:controller.signal});
    if(!r.ok){
...
async function speakRecipeIntentV333(instruction,extra={},after=null){
...
renderStep=function(newStep=false){
  ...
  speakRecipeIntentV333(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token})
};
...
completeRecipeStep=function(s,withPraise=false){
  ...
  if(withPraise){
    ...
    speakRecipeIntentV333(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token},()=>advanceStep(s));
    return
  }
  advanceStep(s)
};
...
advanceStep=function(s){
  ...
  if(s?.afterAi&&state.drink?.id==='mojito'){
    ...
    speakRecipeIntentV333(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase==='recipe'&&state.drink?.id==='mojito')renderStep(true)});
    return
  }
  renderStep(true)
};
...
orderDrink=function(d){
  ...
  speakRecipeIntentV333(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase!=='recipe'||state.drink!==d)return;if(state.difficulty==='hard')askServing();else renderStep(true)})
};
