...
const barTalkHeadState383={active:false,saved:'neutral',timer:null,current:null};
function barTalkDelay383(){return 350+Math.random()*300}
function barTalkNext383(current){
...
function barTalkTick383(){
  const s=barTalkHeadState383;if(!s.active)return;
  const api=window.__barSceneConfig383;
  if(!api?.setHeadVariant){
    s.timer=setTimeout(barTalkTick383,120);
    return
  }
  const current=api.currentHead?.()||s.current||s.saved||'neutral';
  const next=barTalkNext383(current);
  if(api.setHeadVariant(next)){
    s.current=next;
    s.timer=setTimeout(barTalkTick383,barTalkDelay383())
  }else{
    s.timer=setTimeout(barTalkTick383,120)
  }
}
function startTalkingHead383(){
  const s=barTalkHeadState383;if(s.active)return true;
  const api=window.__barSceneConfig383;
...
function stopTalkingHead383(){
  const s=barTalkHeadState383;
  if(s.timer){clearTimeout(s.timer);s.timer=null}
  if(!s.active)return;
  s.active=false;
  const restore=s.saved||'neutral';
  s.saved='neutral';s.current=null;
  window.__barSceneConfig383?.setHeadVariant?.(restore)
}
window.__barSpeechHead383={start:startTalkingHead383,stop:stopTalkingHead383,state:barTalkHeadState383};
...
function stopSpeech(options={}){
  ...
  if(!keepTalkingHead)stopTalkingHead383()
}
async function barPlayTtsV337(text,options={}){
  ...
  const notifyPlayback=()=>{
    ...
    if(manageHead){startTalkingHead383();headStarted=true}
  ...
  try{
    ...
    if(manageHead&&headStarted)stopTalkingHead383()
  }
}
