'use strict';
/* Bar Game 3.95 unified dialogue UI.
...
(function(){
 ...
 function submit(raw,options={}){
   raw=String(raw||'').trim();if(!raw)return;
   const silent=options.silent!==false;
   add(player(),raw);
   if(state.phase==='recipe'&&typeof handleRecipeInput==='function')handleRecipeInput(raw,{silent});
   else if(typeof handleConversation==='function')handleConversation(raw,{silent})
 }
 ...
 function beginTyping(){
   ...
   el.onkeydown=e=>{if(e.key!=='Enter')return;e.preventDefault();let raw=el.textContent||'',p=n+':';raw=raw.startsWith(p)?raw.slice(p.length).trim():raw.trim();
     el.contentEditable='false';el.onkeydown=null;typing=false;d.classList.remove('typing');if(controls)controls.style.display='';submit(raw,{silent:false})}
 }
 function micInput(btn){
   stopForInput();
   if(typeof recognize==='function')recognize(alts=>submit((alts[0]||'').trim(),{silent:false}),btn)
 }
 function makeControls(){
   const c=q('#dialogControls');if(!c)return;
   let row=c.querySelector('.v31Controls');
   if(!row){row=document.createElement('div');row.className='v31Controls';row.innerHTML='<button class="btn secondary" id="v31MicBtn" title="Mikrofon">🎙️</button><button class="btn" id="v31KeyboardBtn" title="Tangentbord">Tangentbord</button><button class="btn secondary" id="v31PrintBtn" title="Utskrift">Utskrift</button>';c.prepend(row)}
