'use strict';
(function(){
  const START_CONFIG_URL='./bartender_installningar_start10h.json';
  const HIGHBALL_CONFIG_URL='./bartender_installningar_highballglas5.json';
  const GARNISH_CONFIG_URL='./bartender_installningar_mojitogarnering4.json';
  ...
  function rotatePoint(x,y,cx,cy,deg){
    const a=Number(deg||0)*Math.PI/180,dx=x-cx,dy=y-cy;
    return {x:cx+dx*Math.cos(a)-dy*Math.sin(a),y:cy+dx*Math.sin(a)+dy*Math.cos(a)};
  }
  function clearGlobalBarGarnish(){
    objectLayer?.querySelectorAll('.globalBarGarnish384').forEach(el=>el.remove());
    clearLayer(barGarnish);
  }
  function addBarGarnishGlobal(p){
    const g=sceneConfig?.diagnostics?.lastHighball;
    if(!objectLayer||!g?.applied||!g?.source||!p?.file)return false;
    const left=Number(g.applied.left),top=Number(g.applied.top),w=Number(g.applied.width),h=Number(g.applied.height);
    const gr=Number(g.source.r||0),gpx=Number(g.source.px??50),gpy=Number(g.source.py??50);
    const pivotX=left+w*gpx/100,pivotY=top+h*gpy/100;
    const localX=left+w*Number(p.x||0)/100,localY=top+h*Number(p.y||0)/100;
    const pos=rotatePoint(localX,localY,pivotX,pivotY,gr);
    const img=document.createElement('img');
    img.src=p.file;
    img.alt='';
    img.className='globalBarGarnish384';
    img.setAttribute('aria-hidden','true');
    img.style.position='absolute';
    img.style.left=pos.x+'px';
    img.style.top=pos.y+'px';
    img.style.width=(w*Number(p.w||0)/100)+'px';
    img.style.height='auto';
    img.style.maxWidth='none';
    img.style.pointerEvents='none';
    img.style.transformOrigin=Number(p.px??50)+'% '+Number(p.py??50)+'%';
    img.style.transform='translate(-50%,-50%) rotate('+(gr+Number(p.r||0))+'deg)';
    img.style.opacity=String(p.opacity??1);
    img.style.filter=Number(p.blur||0)>0?'blur('+Number(p.blur)+'px)':'none';
    img.style.zIndex=String(p.z??45);
    objectLayer.appendChild(img);
    return true;
  }
  function renderBarGarnishGlobal(cfg){
    clearGlobalBarGarnish();
    const items=Object.values(cfg?.glassSystem?.items||{});
    const copies=Array.isArray(cfg?.glassSystem?.copies)?cfg.glassSystem.copies:[];
    let count=0;
    [...items,...copies]
      .sort((a,b)=>Number(a.z||0)-Number(b.z||0))
      .forEach(p=>{if(addBarGarnishGlobal(p))count++});
    return count;
  }
  ...
  function renderGarnish(cfg){
    diagnostics.garnishCountBar=renderBarGarnishGlobal(cfg);
  ...
  const sceneConfig=window.__barSceneConfig384;
  const ready=Promise.all([
    ...
    if(!highballCfg?.objects?.highballGlass)throw new Error('Highballglas5 saknar objects.highballGlass');
    ...
    console.error('Bar v3.84 glass system:',err);
    throw err;
  });
