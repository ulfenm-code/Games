...
/* Bar Game 3.93 AI + cancellable voice-session controller.
...
state.aiActiveVoiceV393=null;
state.voiceGenerationV393=0;
...
function setAiBusyV119(busy,label='Alex svarar…'){
  ...
  /* v3.91: screen choices stay active while the bartender is speaking. */
...
function cancelActiveVoiceV393(){
  state.voiceGenerationV393=(state.voiceGenerationV393||0)+1;
  const active=state.aiActiveVoiceV393;
  state.aiActiveVoiceV393=null;
  if(active&&typeof active.cancel==='function'){
    try{active.cancel()}catch(e){console.error('Voice cancel',e)}
  }else{
    stopSpeech()
  }
}
function createStreamingSpeechV119(){
  cancelActiveVoiceV393();
  const generation=++state.voiceGenerationV393;
  let pending='',chain=Promise.resolve(),streamDone=false,resolved=false,resolveDone,cancelled=false,playbackStarted=false,api=null;
  const done=new Promise(resolve=>{resolveDone=resolve});
  function isCurrent(){return !cancelled&&generation===state.voiceGenerationV393}
  function beginPlayback(){
    if(playbackStarted||!isCurrent())return;
  ...
  function resolveVoice(){
    ...
    if(state.aiActiveVoiceV393===api)state.aiActiveVoiceV393=null;
    resolveDone()
  }
  function maybeDone(){if(!resolved&&streamDone){chain.finally(resolveVoice)}}
  function queue(text){
    const s=String(text||'').trim();if(!s||!isCurrent())return;
    chain=chain.then(()=>{
      if(!isCurrent())return undefined;
      return barPlayTtsV337(s,{manageHead:false,onPlaybackStart:beginPlayback})
  ...
  api={
    push(delta){if(!isCurrent())return;pending+=String(delta||'');flushSentences()},
    finish(){if(!isCurrent())return;if(pending.trim())queue(pending.trim());pending='';streamDone=true;maybeDone()},
    cancel(){
      if(cancelled)return;
      cancelled=true;pending='';streamDone=true;
      if(state.aiActiveVoiceV393===api)state.aiActiveVoiceV393=null;
      stopSpeech();
      if(!resolved){resolved=true;if(playbackStarted)stopTalkingHead381();resolveDone()}
    },
    done,
    generation
  };
  state.aiActiveVoiceV393=api;
  return api
}
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  const controller=new AbortController();state.aiActiveControllerV391=controller;const timer=setTimeout(()=>controller.abort(),22000);const started=performance.now();let first=0;
  try{
...
function interruptAlexV391(_options={}){
...
function alexEventV388(type,facts={},options={}){
  ...
  const interruptEpoch=state.aiInterruptEpochV391||0;
  const task=async()=>{
    ...
    const silent=Boolean(options?.silent)&&options?.forceVoice!==true;
    ...
    try{
      ...
      if(type!=='idle'&&(state.phase==='chat'||state.phase==='recipe')&&!state.aiBusy)scheduleIdle();
      return data
    }catch(e){
      voice?.cancel();
      if(interruptEpoch!==state.aiInterruptEpochV391)return null;
...
handleConversation=async function(raw,options={}){
  if(state.aiBusy)return;markUserActivityV388();const silent=Boolean(options?.silent);const turnEpoch=state.aiInterruptEpochV391||0;const text=String(raw||'').trim();if(!text){toast('Säg eller skriv något först.');return}
  const input=$('#chatInput');if(input)input.value='';$('#dialogControls').innerHTML='';streamDialogV119('');setAiBusyV119(true);const voice=silent?null:createStreamingSpeechV119();const token=`chat:${state.recipeSessionV119}`;
  try{
    const data=await callAiStreamV119(text,{phase:'chat',currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:token},(full,delta)=>{if(turnEpoch!==state.aiInterruptEpochV391)return;streamDialogV119(full);voice?.push(delta)});
    if(turnEpoch!==state.aiInterruptEpochV391){voice?.cancel();return}
    ...
    if(voice){voice.finish();setAiBusyV119(false);streamDialogV119(turn.reply||'');await voice.done}else{setAiBusyV119(false);streamDialogV119(turn.reply||'')};
  ...
  }catch(e){voice?.cancel();if(turnEpoch!==state.aiInterruptEpochV391)return;console.error('Bar 3.91 chat',e);setAiBusyV119(false);aiFallbackV119('Kunde inte nå bartendern. Försök igen.');renderConversationInput(false)}
};

handleRecipeInput=async function(raw,options={}){
  if(state.aiBusy)return;
  markUserActivityV388();
  const silent=Boolean(options?.silent);const turnEpoch=state.aiInterruptEpochV391||0;
  const text=String(raw||'').trim(),s=currentStepV119();if(!s||!text){if(!text)toast('Säg eller skriv något först.');return}
  const snap=recipeSnapshotV119();const input=$('#recipeText');if(input)input.value='';setAiBusyV119(true);streamDialogV119('');const voice=silent?null:createStreamingSpeechV119();
  try{
    const data=await callAiStreamV119(text,{stepToken:snap.token},(full,delta)=>{
      if(turnEpoch!==state.aiInterruptEpochV391)return;
    ...
    if(voice){voice.finish();setAiBusyV119(false)}else setAiBusyV119(false);const turn=data.turn;
    if(turn.context_token!==snap.token){voice?.cancel();renderStep(false);return}
    streamDialogV119(turn.reply||'');if(voice)await voice.done;
  ...
  }catch(e){voice?.cancel();if(turnEpoch!==state.aiInterruptEpochV391)return;console.error('Bar 3.91 recipe',e);setAiBusyV119(false);if(sameRecipeSnapshotV119(snap)){aiFallbackV119('Kunde inte nå bartendern. Försök igen.');renderRecipeControlsV119()}}
};
...
window.__barAI392=window.__barAI391;
window.__barAI393={...window.__barAI391,cancelVoice:cancelActiveVoiceV393,voiceGeneration:()=>state.voiceGenerationV393,activeVoice:()=>state.aiActiveVoiceV393};
