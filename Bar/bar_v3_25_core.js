...
const BAR_TTS_URL_V325='https://azoytlshxfbxbrsqdvvn.supabase.co/functions/v1/bar-bartender-tts-v1';
let barTtsAudioV325=null,barTtsAbortV325=null;
function stopSpeech(){
  try{speechSynthesis.cancel()}catch(_){}
  try{barTtsAbortV325?.abort()}catch(_){}
  barTtsAbortV325=null;
  try{if(barTtsAudioV325){barTtsAudioV325.pause();barTtsAudioV325.src=''}}catch(_){}
  barTtsAudioV325=null
}
async function barPlayTtsV325(text){
  const s=String(text||'').trim();if(!s)return;
  stopSpeech();const controller=new AbortController();barTtsAbortV325=controller;
  const r=await fetch(BAR_TTS_URL_V325,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({input:s}),signal:controller.signal});
  if(!r.ok)throw new Error('TTS HTTP '+r.status);
  const blob=await r.blob();if(controller.signal.aborted)return;
  const url=URL.createObjectURL(blob),audio=new Audio(url);barTtsAudioV325=audio;
  await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=reject;audio.play().catch(reject)});
  URL.revokeObjectURL(url);if(barTtsAudioV325===audio)barTtsAudioV325=null;if(barTtsAbortV325===controller)barTtsAbortV325=null
}
function speak(text,onEnd){
  let done=false;const finish=()=>{if(done)return;done=true;if(onEnd)onEnd()};
  barPlayTtsV325(text).then(finish).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI TTS',e);finish()})
}
