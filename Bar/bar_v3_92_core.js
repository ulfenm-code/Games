...
(function(){
  ...
  function activateWhiteRumV347(){
    ...
    if(correctQuestion){
      window.__barAI392?.interrupt?.({resumeVoice:true});
      window.__barAI392?.activity?.();
      window.__barAI392?.event?.('correct_answer',{answer:step.a,source:'bottle_choice'},{
        target:'#dialogText',phase:'recipe',forceVoice:true,
        extra:{currentQuestion:step.q,expectedAnswer:step.a}
      });
    }
    bottleBusy=true;
