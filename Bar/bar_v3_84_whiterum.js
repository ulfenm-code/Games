...
(function(){
  ...
  const MOVE_MS=1500;
  const scene=window.__barSceneConfig384;
  let busy=false;
  let rafId=0;
  ...
  async function playCorrect(){
    ...
    if(!scene?.loadAndApply||!scene?.loadConfig||!scene?.renderPoseFrame)throw new Error('White Rum-animation saknar v3.84 scenmotor');
    busy=true;
    try{
      setCapHidden(true);
      ...
      setCapHidden(false);
      console.error('Bar v3.84 White Rum animation:',err);
      throw err;
    }finally{
      busy=false;
    }
