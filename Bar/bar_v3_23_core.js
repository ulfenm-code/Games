...
{q:'Vad ska läggas till?',a:'Socker och limejuice',opts:['Socker och limejuice','Salt och citronjuice','Socker och apelsinjuice']},
...
const BAR_TTS_URL_V323='https://azoytlshxfbxbrsqdvvn.supabase.co/functions/v1/bar-bartender-tts-v1';
let barTtsAudioV323=null,barTtsAbortV323=null;
function stopSpeech(){
  try{speechSynthesis.cancel()}catch(_){}
  try{barTtsAbortV323?.abort()}catch(_){}
  barTtsAbortV323=null;
  try{if(barTtsAudioV323){barTtsAudioV323.pause();barTtsAudioV323.src=''}}catch(_){}
  barTtsAudioV323=null
}
async function barPlayTtsV323(text){
  const s=String(text||'').trim();if(!s)return;
  stopSpeech();const controller=new AbortController();barTtsAbortV323=controller;
  const r=await fetch(BAR_TTS_URL_V323,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({input:s}),signal:controller.signal});
  if(!r.ok)throw new Error('TTS HTTP '+r.status);
  const blob=await r.blob();if(controller.signal.aborted)return;
  const url=URL.createObjectURL(blob),audio=new Audio(url);barTtsAudioV323=audio;
  await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=reject;audio.play().catch(reject)});
  URL.revokeObjectURL(url);if(barTtsAudioV323===audio)barTtsAudioV323=null;if(barTtsAbortV323===controller)barTtsAbortV323=null
}
function speak(text,onEnd){
  let done=false;const finish=()=>{if(done)return;done=true;if(onEnd)onEnd()};
  barPlayTtsV323(text).then(finish).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI TTS',e);finish()})
}
