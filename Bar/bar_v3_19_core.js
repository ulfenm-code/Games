...
const drinks=[
{id:'mojito',name:'Mojito',minAge:18,alcoholic:true,intro:'',steps:[
{intent:'glass',q:'Vilket glas?',a:'Highballglas',opts:['Highballglas','Martiniglas','Shotglas']},
{intent:'herb',q:'Vilken ört?',a:'Mynta',opts:['Mynta','Basilika','Rosmarin']},
{intent:'sugar_lime',q:'Vad lägger vi till?',a:'Socker och limejuice',opts:['Socker och limejuice','Salt och citronjuice','Socker och apelsinjuice']},
{intent:'mix',q:'Vad gör vi nu?',a:'Blanda',opts:['Blanda','Skaka','Sila']},
{intent:'soda_splash',auto:true,note:'splash_soda'},
{intent:'spirit',q:'Vilken sprit?',a:'Vit kubansk rom',opts:['Vit kubansk rom','Gin','Tequila'],amount:{base:4.5,unit:'cl',label:'vit kubansk rom'}},
{intent:'top',q:'Vad toppar vi med?',a:'Sodavatten',opts:['Sodavatten','Cola','Tonic']},
{intent:'finish',q:'Vad gör vi nu?',a:'Rör om lätt',opts:['Rör om lätt','Skaka','Sila']},
{intent:'garnish',q:'Vad garnerar vi med? Två saker.',a:'Mynta och lime',opts:['Mynta och lime','Citron och basilika','Apelsin och rosmarin']}
]},
...
function isMojitoV319(){return state.drink?.id==='mojito'}
function mojitoContextV319(s){return isMojitoV319()?{recipeSource:'IBA',drink:'Mojito',intent:s?.intent||null,dialogueRule:'Formulera naturligt på svenska utifrån intent och facit. Använd aldrig frasen nästa fråga. Använd inte ordet bassprit. Hårdkodade q-fält är endast semantiska fallbacketiketter, inte bartenderrepliker.',iba:{ingredients:['45 ml White Cuban Ron','20 ml Fresh Lime Juice','6 pcs Mint Sprigs','2 tsp White Cane Sugar','Soda Water'],method:['mix mint with sugar and lime juice','add splash of soda water and fill glass with ice','pour rum','top with soda water','light stir'],garnish:['mint sprigs','slice of lime']}}:null}
...
function speak(text,onEnd){
  try{
    ...
    speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(String(text||''));u.lang='sv-SE';u.rate=.96;u.pitch=.96;
    if(onEnd){let done=false;const finish=()=>{if(done)return;done=true;onEnd()};u.onend=finish;u.onerror=finish;setTimeout(finish,Math.max(2200,Math.min(9000,900+String(text||'').length*58)))}
    speechSynthesis.speak(u)
  }catch(_){if(onEnd)setTimeout(onEnd,1000)}
}
...
function renderStep(newStep=false){
  ...
  if(isMojitoV319()&&s?.auto){state.step++;state.lastRecipeQuestion='';if(typeof renderStep==='function')return renderStep(true);return}
  state.lastRecipeQuestion=naturalQuestion(s);$('#dialogText').textContent=state.lastRecipeQuestion;speak(state.lastRecipeQuestion);
