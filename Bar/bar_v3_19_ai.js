...
function aiGameContextV119(extra={}){
  ...
  return {
    phase,
    recipeAuthority:isMojitoV319()?'IBA':null,
    mojito:isMojitoV319()?mojitoContextV319(s):null,
    orderingRule:phase==='chat'?'If the player clearly asks for Mojito, return action order_drink with drink_id mojito. For any listed drink use its exact id.':null,
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  ...
  try{
    ...
    const r=await fetch(BAR_AI_URL_V119,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({message,previous_response_id:state.aiPreviousResponseId,game:context}),signal:controller.signal});
    if(!r.ok){
...
renderStep=function(newStep=false,prefix=''){
  ...
  if(isMojitoV319()&&s?.auto){
    const snap=recipeSnapshotV119();setAiBusyV119(true);streamDialogV119('');const voice=createStreamingSpeechV119();
    callAiStreamV119('Fortsätt Mojito-samtalet naturligt. Berätta kort att du nu brukar hälla i en liten skvätt sodavatten. Ställ ingen fråga i denna replik.',{stepToken:snap.token,currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[]},(full,delta)=>{if(sameRecipeSnapshotV119(snap)){streamDialogV119(full);voice.push(delta)}})
      .then(async data=>{if(!sameRecipeSnapshotV119(snap)){voice.cancel();setAiBusyV119(false);return}voice.finish();setAiBusyV119(false);streamDialogV119(data.turn.reply||'');await voice.done;if(!sameRecipeSnapshotV119(snap))return;state.step++;state.lastRecipeQuestion='';renderStep(true)})
      .catch(e=>{voice.cancel();console.error('Mojito transition',e);setAiBusyV119(false);state.step++;state.lastRecipeQuestion='';renderStep(true)});
    return;
  }
  if(isMojitoV319()){
    const snap=recipeSnapshotV119();setAiBusyV119(true);streamDialogV119('');const voice=createStreamingSpeechV119();
    callAiStreamV119('Fortsätt Mojito-samtalet naturligt och ställ frågan som hör till aktuellt intent. Frågan ska kännas som ett samtal mellan två människor. Säg aldrig "nästa fråga". Använd inte ordet "bassprit". Ge inte bort svaret.',{stepToken:snap.token},(full,delta)=>{if(sameRecipeSnapshotV119(snap)){streamDialogV119(full);voice.push(delta)}})
      .then(async data=>{if(!sameRecipeSnapshotV119(snap)){voice.cancel();setAiBusyV119(false);return}voice.finish();setAiBusyV119(false);state.lastRecipeQuestion=data.turn.reply||'';streamDialogV119(state.lastRecipeQuestion);renderRecipeControlsV119();await voice.done})
      .catch(e=>{voice.cancel();console.error('Mojito question',e);setAiBusyV119(false);renderRecipeControlsV119()});
    return;
  }
...
orderDrink=function(d){
  ...
  if(isMojitoV319()){renderStep(true);return}
...
handleConversation=async function(raw){
  ...
  const low=text.toLowerCase();if(low.includes('mojito')&&/(sugen|vill|beställ|gör|blanda|ta|ha|dricka)/i.test(text)){orderDrinkById('mojito');return}
  ...
  }catch(e){voice.cancel();console.error('Bar 1.19 chat',e);setAiBusyV119(false);streamDialogV119('Jag fick inget svar från bartenderns AI. Försök igen.');renderConversationInput(false)}
};
