...
const BAR_TTS_URL_V329='https://azoytlshxfbxbrsqdvvn.supabase.co/functions/v1/bar-bartender-tts-v1';
let barTtsAudioV329=null,barTtsAbortV329=null;
function stopSpeech(){
  try{speechSynthesis.cancel()}catch(_){}
  try{barTtsAbortV329?.abort()}catch(_){}
  barTtsAbortV329=null;
  try{if(barTtsAudioV329){barTtsAudioV329.pause();barTtsAudioV329.src=''}}catch(_){}
  barTtsAudioV329=null
}
async function barPlayTtsV329(text){
  const s=String(text||'').trim();if(!s)return;
  stopSpeech();const controller=new AbortController();barTtsAbortV329=controller;
  const r=await fetch(BAR_TTS_URL_V329,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({input:s}),signal:controller.signal});
  if(!r.ok)throw new Error('TTS HTTP '+r.status);
  const blob=await r.blob();if(controller.signal.aborted)return;
  const url=URL.createObjectURL(blob),audio=new Audio(url);barTtsAudioV329=audio;
  await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=reject;audio.play().catch(reject)});
  URL.revokeObjectURL(url);if(barTtsAudioV329===audio)barTtsAudioV329=null;if(barTtsAbortV329===controller)barTtsAbortV329=null
}
function speak(text,onEnd){
  let done=false;const finish=()=>{if(done)return;done=true;if(onEnd)onEnd()};
  barPlayTtsV329(text).then(finish).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI TTS',e);finish()})
}
