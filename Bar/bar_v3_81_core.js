...
const barTalkHeadState381={active:false,saved:'neutral',timer:null,current:null};
function barTalkDelay381(){return 350+Math.random()*300}
function barTalkNext381(current){
...
function barTalkTick381(){
  const s=barTalkHeadState381;if(!s.active)return;
  const api=window.__barSceneConfig381;
  if(!api?.setHeadVariant){
    s.timer=setTimeout(barTalkTick381,120);
    return
  }
  const current=api.currentHead?.()||s.current||s.saved||'neutral';
  const next=barTalkNext381(current);
  if(api.setHeadVariant(next)){
    s.current=next;
    s.timer=setTimeout(barTalkTick381,barTalkDelay381())
  }else{
    s.timer=setTimeout(barTalkTick381,120)
  }
}
function startTalkingHead381(){
  const s=barTalkHeadState381;if(s.active)return true;
  const api=window.__barSceneConfig381;
...
function stopTalkingHead381(){
  const s=barTalkHeadState381;
  if(s.timer){clearTimeout(s.timer);s.timer=null}
  if(!s.active)return;
  s.active=false;
  const restore=s.saved||'neutral';
  s.saved='neutral';s.current=null;
  window.__barSceneConfig381?.setHeadVariant?.(restore)
}
window.__barSpeechHead381={start:startTalkingHead381,stop:stopTalkingHead381,state:barTalkHeadState381};
...
function stopSpeech(options={}){
  ...
  if(!keepTalkingHead)stopTalkingHead381()
}
async function barPlayTtsV337(text,options={}){
  ...
  const notifyPlayback=()=>{
    ...
    if(manageHead){startTalkingHead381();headStarted=true}
  ...
  try{
    ...
    if(manageHead&&headStarted)stopTalkingHead381()
  }
}
