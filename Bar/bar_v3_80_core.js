...
const barTalkHeadState380={active:false,saved:'neutral',timer:null,lastMouth:null,lastSurpriseAt:0,current:null};
function barTalkDelay380(min,max){return min+Math.random()*(max-min)}
function barTalkTick380(){
  const s=barTalkHeadState380;if(!s.active)return;
  const api=window.__barSceneConfig380;
  if(!api?.setHeadVariant){s.timer=setTimeout(barTalkTick380,180);return}
  const now=Date.now(),surprise=(now-s.lastSurpriseAt>=1500&&Math.random()<0.05);
  let next,delay;
  if(surprise){
    next='forvanad';s.lastSurpriseAt=now;delay=barTalkDelay380(280,400);
  }else{
    next=s.lastMouth==='pratar1'?'pratar2':s.lastMouth==='pratar2'?'pratar1':(Math.random()<0.5?'pratar1':'pratar2');
    s.lastMouth=next;delay=barTalkDelay380(140,260);
  }
  if(api.setHeadVariant(next)){s.current=next}
  s.timer=setTimeout(barTalkTick380,delay);
}
function startTalkingHead380(){
  const s=barTalkHeadState380;if(s.active)return true;
  const api=window.__barSceneConfig380;if(!api?.setHeadVariant)return false;
  s.saved=api.currentHead?.()||'neutral';s.active=true;s.lastMouth=null;s.lastSurpriseAt=0;s.current=null;
  barTalkTick380();return true
}
function stopTalkingHead380(){
  const s=barTalkHeadState380;
  if(s.timer){clearTimeout(s.timer);s.timer=null}
  if(!s.active)return;
  s.active=false;
  const restore=s.saved||'neutral';s.saved='neutral';s.lastMouth=null;s.current=null;
  window.__barSceneConfig380?.setHeadVariant?.(restore)
}
window.__barSpeechHead380={start:startTalkingHead380,stop:stopTalkingHead380,state:barTalkHeadState380};
...
function stopSpeech(options={}){
  ...
  if(!keepTalkingHead)stopTalkingHead380()
}
async function barPlayTtsV337(text,options={}){
  ...
  if(manageHead)startTalkingHead380();
  ...
  try{
    ...
    await new Promise((resolve,reject)=>{
      ...
      const cleanup=()=>{audio.onended=null;audio.onerror=null;controller.signal.removeEventListener('abort',onAbort)};
    ...
    if(manageHead)stopTalkingHead380()
  }
}
...
function sayOrderUnavailable(text){
  $('#dialogText').textContent=text;
  speak(text,()=>{if(state.phase==='chat')renderConversationInput(false)});
}
function orderDrinkById(id){
  const menuItem=drinkMenuCatalog.find(x=>x.id===id);
  if(!menuItem){sayOrderUnavailable('Den kan jag inte göra här.');return false}
  if(!menuItem.buildable){
    sayOrderUnavailable(menuItem.name+' finns på drinklistan, men just nu kan jag inte göra den. Hoppas kunna göra den snart.');
    return false
  }
  const d=drinks.find(x=>x.id===id);
  if(!d){sayOrderUnavailable('Den kan jag inte göra just nu.');return false}
