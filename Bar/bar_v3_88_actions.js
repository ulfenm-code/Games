...
function startPhysicalAction(type,afterAction){
  ...
  const names={muddle:'Muddla mynta och lime',shake:'Skaka drinken',drink:'Smaka på drinken'};
...
function showPhysicalStage(){
  ...
  if(state.action==='muddle'){
    ...
    alexActionEventV388('physical_instruction',{action:'muddle',goal:'Spelaren ska trycka och gnugga mynta och lime försiktigt med fingret, inte mosa sönder dem.'})
...
function addMuddle(){
  const now=performance.now();if(now-state.lastMuddle<55)return;state.lastMuddle=now;state.muddleProgress=Math.min(100,state.muddleProgress+7);
  $('#muddleGlass').classList.add('hit');setTimeout(()=>$('#muddleGlass').classList.remove('hit'),90);updateMuddle();if(state.muddleProgress>=100)finishPhysical('muddle')
}
$('#muddleGlass').onpointerdown=e=>{e.preventDefault();addMuddle()};
$('#muddleGlass').onpointermove=e=>{if(e.buttons||e.pointerType==='touch'){e.preventDefault();addMuddle()}};
$('#muddleFallback').onclick=()=>{state.muddleProgress=Math.min(100,state.muddleProgress+18);updateMuddle();if(state.muddleProgress>=100)finishPhysical('muddle')};
...
window.addEventListener('devicemotion',e=>{
  ...
  if(mag>15&&now-state.lastMotion>95){state.lastMotion=now;state.shakeProgress=Math.min(100,state.shakeProgress+7);updateShake();if(state.shakeProgress>=100)finishPhysical('shake')}
});
