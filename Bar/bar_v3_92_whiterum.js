'use strict';
(function(){
  const START_URL='./bartender_installningar_whiterum_start&stop3.json';
  const POUR_URL='./bartender_installningar_whiterum_poor3.json';
  const HIGHBALL_URL='./bartender_installningar_higballglas6.json';
  ...
  async function playCorrect(){
    ...
    try{
      ...
      const startResult=await scene.loadAndApply(START_URL,{showGlass:null,preserveCanonicalHead:false});
      ...
      const endResult=await scene.loadAndApply(POUR_URL,{showGlass:null,preserveCanonicalHead:false});
      ...
      const returnResult=await scene.loadAndApply(HIGHBALL_URL,{showGlass:true});
      if(!returnResult?.applied||scene.activeUrl!==HIGHBALL_URL)throw new Error('Highballglas6 kunde inte aktiveras efter White Rum-animationen');
      setCapHidden(false);
      return true;
    }catch(err){
      setCapHidden(false);
      console.error('Bar v3.88 White Rum animation:',err);
      throw err;
    }finally{
      busy=false;
    }
