...
const BAR_TTS_INSTRUCTION_V329='Tala naturlig svenska, varm manlig bartender-röst, avslappnad och vänlig.';
...
function createStreamingSpeechV119(){
  let pending='',chain=Promise.resolve(),streamDone=false,resolved=false,resolveDone,cancelled=false;
  const done=new Promise(resolve=>{resolveDone=resolve});
  stopSpeech();
  function maybeDone(){if(!resolved&&streamDone){chain.finally(()=>{if(!resolved){resolved=true;resolveDone()}})}}
  function queue(text){const s=String(text||'').trim();if(!s||cancelled)return;chain=chain.then(()=>cancelled?undefined:barPlayTtsV329(s)).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI streaming TTS',e)})}
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  ...
  try{
    ...
    const r=await fetch(BAR_AI_URL_V119,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({message,previous_response_id:state.aiPreviousResponseId,game:context,voice_instruction:BAR_TTS_INSTRUCTION_V329}),signal:controller.signal});
    if(!r.ok){
...
function renderRecipeControlsV119(){
  ...
  let header='';
  if(direct)header='<div class="hintTitle">Välj ett av alternativen</div>';
  else if(threeErrors)header='<div class="hintTitle">Tre fel – här får du tre alternativ. Ett är rätt.</div>';
...
async function speakRecipeIntentV329(instruction,extra={},after=null){
...
renderStep=function(newStep=false){
  ...
  const s=currentStepV119();state.lastRecipeQuestion=s.q;renderRecipeControlsV119();
  const token=recipeTokenV119('recipe');
  const instruction='Ställ nästa receptfråga naturligt med egna ord. Frågans innebörd är: '+s.q+' Säg inte "nästa fråga", "nästa steg" eller någon lagrad övergångsfras. Fråga kort och som en bartender i ett naturligt samtal.';
  speakRecipeIntentV329(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token})
};
...
registerWrongAnswer=function(s,preserveDialog=false){
  ...
  if(!preserveDialog){
    const token=recipeTokenV119('recipe');
    const instruction='Spelarens svar var fel. Reagera kort och naturligt med egna ord utan lagrad standardfras. Uppmuntra ett nytt försök. Avslöja inte rätt svar.';
    speakRecipeIntentV329(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token})
  }
};
...
completeRecipeStep=function(s,withPraise=false){
  ...
  if(withPraise){
    const token=recipeTokenV119('recipe');
    const instruction='Spelarens svar var rätt. Bekräfta det kort och naturligt med egna ord. Använd ingen lagrad berömfras. Ställ inte nästa receptfråga i samma replik.';
    speakRecipeIntentV329(instruction,{currentQuestion:s.q,expectedAnswer:s.a,stepToken:token},()=>advanceStep(s));
    return
  }
  advanceStep(s)
};
...
advanceStep=function(s){
  ...
  if(s?.afterAi&&state.drink?.id==='mojito'){
    ...
    speakRecipeIntentV329(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase==='recipe'&&state.drink?.id==='mojito')renderStep(true)});
    return
  }
  renderStep(true)
};
...
orderDrink=function(d){
  ...
  const instruction='Spelaren har beställt '+d.name+'. Bekräfta beställningen naturligt och kort och berätta att spelaren ska hjälpa till bakom baren. Formulera hela repliken själv. Använd ingen lagrad standardfras.';
  speakRecipeIntentV329(instruction,{currentQuestion:null,expectedAnswer:null,answerMode:null,answerParts:[],alreadyCorrectParts:[],stepToken:recipeTokenV119('recipe')},()=>{if(state.phase!=='recipe'||state.drink!==d)return;if(state.difficulty==='hard')askServing();else renderStep(true)})
};
