...
const BAR_TTS_URL_V333='https://azoytlshxfbxbrsqdvvn.supabase.co/functions/v1/bar-bartender-tts-v1';
let barTtsAudioV333=null,barTtsAbortV333=null;
function stopSpeech(){
  try{speechSynthesis.cancel()}catch(_){}
  try{barTtsAbortV333?.abort()}catch(_){}
  barTtsAbortV333=null;
  try{if(barTtsAudioV333){barTtsAudioV333.pause();barTtsAudioV333.src=''}}catch(_){}
  barTtsAudioV333=null
}
async function barPlayTtsV333(text){
  const s=String(text||'').trim();if(!s)return;
  stopSpeech();const controller=new AbortController();barTtsAbortV333=controller;
  const r=await fetch(BAR_TTS_URL_V333,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({input:s}),signal:controller.signal});
  if(!r.ok)throw new Error('TTS HTTP '+r.status);
  const blob=await r.blob();if(controller.signal.aborted)return;
  const url=URL.createObjectURL(blob),audio=new Audio(url);barTtsAudioV333=audio;
  await new Promise((resolve,reject)=>{audio.onended=resolve;audio.onerror=reject;audio.play().catch(reject)});
  URL.revokeObjectURL(url);if(barTtsAudioV333===audio)barTtsAudioV333=null;if(barTtsAbortV333===controller)barTtsAbortV333=null
}
function speak(text,onEnd){
  let done=false;const finish=()=>{if(done)return;done=true;if(onEnd)onEnd()};
  barPlayTtsV333(text).then(finish).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI TTS',e);finish()})
}
