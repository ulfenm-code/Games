...
const barTalkHeadState382={active:false,saved:'neutral',timer:null,current:null};
function barTalkDelay382(){return 350+Math.random()*300}
function barTalkNext382(current){
...
function barTalkTick382(){
  const s=barTalkHeadState382;if(!s.active)return;
  const api=window.__barSceneConfig382;
  if(!api?.setHeadVariant){
    s.timer=setTimeout(barTalkTick382,120);
    return
  }
  const current=api.currentHead?.()||s.current||s.saved||'neutral';
  const next=barTalkNext382(current);
  if(api.setHeadVariant(next)){
    s.current=next;
    s.timer=setTimeout(barTalkTick382,barTalkDelay382())
  }else{
    s.timer=setTimeout(barTalkTick382,120)
  }
}
function startTalkingHead382(){
  const s=barTalkHeadState382;if(s.active)return true;
  const api=window.__barSceneConfig382;
...
function stopTalkingHead382(){
  const s=barTalkHeadState382;
  if(s.timer){clearTimeout(s.timer);s.timer=null}
  if(!s.active)return;
  s.active=false;
  const restore=s.saved||'neutral';
  s.saved='neutral';s.current=null;
  window.__barSceneConfig382?.setHeadVariant?.(restore)
}
window.__barSpeechHead382={start:startTalkingHead382,stop:stopTalkingHead382,state:barTalkHeadState382};
...
function stopSpeech(options={}){
  ...
  if(!keepTalkingHead)stopTalkingHead382()
}
async function barPlayTtsV337(text,options={}){
  ...
  const notifyPlayback=()=>{
    ...
    if(manageHead){startTalkingHead382();headStarted=true}
  ...
  try{
    ...
    if(manageHead&&headStarted)stopTalkingHead382()
  }
}
