...
const BAR_TTS_INSTRUCTION_V323='Tala naturlig svenska, varm manlig bartender-röst, avslappnad och vänlig.';
...
function createStreamingSpeechV119(){
  ...
  function queue(text){const s=String(text||'').trim();if(!s||cancelled)return;chain=chain.then(()=>cancelled?undefined:barPlayTtsV323(s)).catch(e=>{if(e?.name!=='AbortError')console.error('OpenAI streaming TTS',e)})}
...
async function callAiStreamV119(message,extra={},onReply=()=>{},allowRetry=true){
  ...
  try{
    ...
    const r=await fetch(BAR_AI_URL_V119,{method:'POST',headers:{'Content-Type':'application/json','apikey':BAR_SUPABASE_KEY_V119},body:JSON.stringify({message,previous_response_id:state.aiPreviousResponseId,game:context,voice_instruction:BAR_TTS_INSTRUCTION_V323}),signal:controller.signal});
    if(!r.ok){
